...
$cfg = @{Unity=$Unity;Project=$repo;Vive=$Vive;SteamVR=$SteamVR;Usbipd='C:\Program Files\usbipd-win\usbipd.exe';Distro=$Distro;Backend="$Workspace/piper_launcher/entry.sh";WslHome=$wslHome;UnityOnly=[bool]$UnityOnly}
...
if (!(Test-Path -LiteralPath $ShortcutDirectory -PathType Container)) { throw 'Shortcut directory does not exist.' }
$shortcut = $shell.CreateShortcut((Join-Path $ShortcutDirectory 'SURF 一键启动.lnk'))
