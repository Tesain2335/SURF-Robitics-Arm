...
// Adapted from xxStevexxx/Unity-display-for-SURF, a298c2d (2026-09-17).
namespace SURF.Risk
{
...
public sealed class HeadsetSafetyWarningOverlay : MonoBehaviour
{
    ...
    public void SetCamera(Camera camera) { headsetCamera = camera; }
    private void OnEnable() { if (uiRoot) uiRoot.gameObject.SetActive(true); }
    private void OnDisable() { if (uiRoot) uiRoot.gameObject.SetActive(false); }
    private void OnDestroy() { if (uiRoot) Destroy(uiRoot.gameObject); zoneUis.Clear(); }
    ...
    private void BuildCanvas()
    {
        ...
        // Display only: no GraphicRaycaster or UI input interception.
    ...
    private void AttachCanvasToCamera()
    {
        ...
        headsetCamera.cullingMask |= 1 << overlayLayer;
    }
    ...
    private void RefreshZones()
    {
        ...
        foreach (var entry in zoneUis)
            if (!entry.Key || zones == null || System.Array.IndexOf(zones, entry.Key) < 0) removedZones.Add(entry.Key);
        foreach (var zone in removedZones) { zoneUis[zone].Dispose(); zoneUis.Remove(zone); }
    ...
    private void UpdateZoneFrames()
    {
        ...
        for (int i = 0; i < zones.Length; i++)
        {
            ...
            if (!zone.isActiveAndEnabled || !TryGetScreenRect(zone.GetWorldBounds(), out Rect rect))
            {
                ui.SetVisible(false);
                continue;
            }
    ...
    private static string GetZoneLabel(SafetyTemperatureZone zone)
    {
        ...
        string source = zone.IsSimulated ? "SIM" : "Manual / external";
        return zone.Level == SafetyTemperatureLevel.Unknown ? $"{zone.ZoneName} [Unknown]" : $"{zone.ZoneName}\n{levelName}\n{source} {zone.AverageTemperatureC:F1} C";
    }
    ...
    private ZoneUi CreateZoneUi(string zoneName)
    {
        ...
        Text label = CreateText(root, "Label", string.Empty, 16, TextAnchor.LowerLeft, Color.white);
        RectTransform labelRect = label.rectTransform;
        labelRect.anchorMin = new Vector2(0f, 1f);
        labelRect.anchorMax = new Vector2(1f, 1f);
        labelRect.pivot = new Vector2(0f, 0f);
        labelRect.anchoredPosition = new Vector2(0f, 8f);
        labelRect.sizeDelta = new Vector2(0f, 130f);
    ...
    private Image CreateFrameEdge(RectTransform parent, string edgeName)
    {
        ...
        var image = edgeObject.AddComponent<Image>(); image.raycastTarget = false; return image;
    }
    ...
    private void BuildLegend()
    {
        ...
        panel.sizeDelta = new Vector2(370f, 150f);

        Image background = panelObject.AddComponent<Image>();
        background.raycastTarget = false;
        background.color = new Color(0f, 0f, 0f, 0.62f);

        Text title = CreateText(panel, "Title", "SIMULATED TEMPERATURE", 22, TextAnchor.LowerLeft, Color.white);
        ...
        CreateLegendRow(panel, 2, SafetyTemperatureLevel.Danger, "65-120 C Danger / Warning");
        CreateLegendRow(panel, 3, SafetyTemperatureLevel.Critical, ">=120 C Critical / Warning");
    ...
    private void CreateLegendRow(RectTransform parent, int index, SafetyTemperatureLevel level, string text)
    {
        ...
        Text rowText = CreateText(parent, $"Legend {level} Text", text, 18, TextAnchor.LowerLeft, Color.white);
