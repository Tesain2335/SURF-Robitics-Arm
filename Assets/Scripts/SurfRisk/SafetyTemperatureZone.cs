...
public sealed class SafetyTemperatureZone : MonoBehaviour
{
    ...
    public bool IsSimulated => simulateTemperature;
    ...
    {
        if (float.IsNaN(temperatureC) || float.IsInfinity(temperatureC))
            return SafetyTemperatureLevel.Unknown;
        if (temperatureC < T1C)
    ...
    public static Color ColorForLevel(SafetyTemperatureLevel level)
    {
        switch (level)
        {
            case SafetyTemperatureLevel.Unknown:
                return Color.gray;
